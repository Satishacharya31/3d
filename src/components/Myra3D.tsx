...
function Model({ isSpeaking, isThinking, onHeadBoneLoaded }: ModelProps) {
  // Load the GLTF model. It's safe to assume it's at /myra.glb
  const { scene, materials, animations } = useGLTF('/myra.glb');
  ...
  useEffect(() => {
    scene.traverse(child => {
      ...
      // Upper arm should match "arm" or "shoulder" but exclude forearms and hands
      ...
      // Forearm should match "forearm", "lowerarm", or "elbow"
  ...
  // Report head bone back to parent once resolved
  useEffect(() => {
  ...
  // Load standard skeletal animations (guessing talking/thinking/idle)
  useEffect(() => {
  ...
  // Expressive physical bone animation loop
  useFrame((state) => {
    if (group.current) {
      const time = state.clock.elapsedTime;

      // Skeletal gestures and breathing loops
      if (isSpeaking) {
         const gestureCycle = (time * 0.6) % 3; // Cycles between beautiful gesturing poses
         
         // Smooth speaking targets
         let lArmZ = -1.1; 
         let lArmX = -0.25;
         let lArmY = 0.15;
         let lForeX = 1.25;
         ...
         if (gestureCycle < 1.0) {
           // Pose 1: Symmetrical welcoming hands, active high explains
           ...
           // Pose 2: Left arm raised extremely high/speaking, right arm relaxed
           ...
           // Pose 3: Right arm raised extremely high/speaking, left arm relaxed
           const wave = Math.sin(time * 6.0) * 0.35;
           lArmZ = -1.3;
           lArmX = -0.15;
           lArmY = 0.1;
         ...
         // Interpolate arm bones rotation
         ...
         if (headRef.current) {
           headRef.current.rotation.x = THREE.MathUtils.lerp(headRef.current.rotation.x, Math.sin(time * 2.5) * 0.06, 0.09);
           headRef.current.rotation.y = THREE.MathUtils.lerp(headRef.current.rotation.y, Math.sin(time * 1.2) * 0.1, 0.09);
         }
      } else {
         // Gentle idle posture breathing cyclic loop
         ...
         if (headRef.current) {
           headRef.current.rotation.x = THREE.MathUtils.lerp(headRef.current.rotation.x, breathe * 0.2, 0.05);
           headRef.current.rotation.y = THREE.MathUtils.lerp(headRef.current.rotation.y, 0, 0.05);
         }
      }

      if (isThinking && !isSpeaking) {
         group.current.rotation.y = Math.sin(time * 2.0) * 0.2;
      } else {
...
export function Myra3D({ isSpeaking, isThinking }: { isSpeaking: boolean, isThinking?: boolean }) {
  const [headBone, setHeadBone] = useState<THREE.Object3D | null>(null);

  // Position calibrations
  const [mouthX, setMouthX] = useState(0.0);
  const [mouthY, setMouthY] = useState(-0.065);
  const [mouthZ, setMouthZ] = useState(0.101);
  const [mouthScale, setMouthScale] = useState(1.0);

  // Rotation calibrations (Pitch / Yaw / Roll)
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-[100]">
      <Canvas camera={{ position: [0, 0.2, 3], fov: 45 }} gl={{ alpha: true }}>
      ...
      {/* Floating Calibration Slider Controller */}
      <div className="absolute top-20 right-4 pointer-events-auto flex flex-col items-end gap-2 z-[110]">
        <button
        ...
        {isOpen && (
          <div className="w-72 max-h-[440px] overflow-y-auto p-4 bg-black/85 border border-white/10 rounded-2xl backdrop-blur-lg flex flex-col gap-4 shadow-2xl animate-in slide-in-from-top-2 duration-200 scrollbar-thin scrollbar-thumb-white/10">
            ...
            <button
              ...
              className="text-[9px] text-pink-400 hover:text-pink-300 uppercase tracking-widest font-bold text-center mt-2 pt-2.5 border-t border-white/5 w-full flex items-center justify-center hover:underline transition-all font-sans"
            >
              Reset to Defaults
            </button>
          </div>
